...
namespace PowerFx.Dataverse.Cdp.Tests
{
    public class CdpTests
    {
        ...
        public async Task Cdp_Sql()
        {
            ...
            Assert.Equal<object>(@"FieldAccess(__retrieveSingle:r!(Delegable(ResolvedObject('Products:RuntimeValues_XXXX'), __eq:r*, Scope 1(Delegable(ResolvedObject('Products:RuntimeValues_XXXX'), {fieldName: ""ProductID"":s}, {fieldFunctions: Table:*[Value:n]()}, 680:w), __noop:N(), ResolvedObject(__noJoin()), """":s), Name)", ir);
        ...
        public async Task Cdp_Sql_NonSortable()
        {
            ...
            string ir = Regex.Replace(check.PrintIR(), "RuntimeValues_[0-9]+", "RuntimeValues_XXXX");
            Assert.Equal<object>(@"FieldAccess(First:r!(FirstN:r*(__retrieveMultiple:r*, Scope 1(Delegable(ResolvedObject('Products:RuntimeValues_XXXX'), __eq:r*, Scope 1(Delegable(ResolvedObject('Products:RuntimeValues_XXXX'), {fieldName: ""ProductID"":s}, {fieldFunctions: Table:*[Value:n]()}, 680:w), __noop:N(), ResolvedObject(__noJoin()), 1000:n, """":s), Float:n(2:w))), Name)", ir);
