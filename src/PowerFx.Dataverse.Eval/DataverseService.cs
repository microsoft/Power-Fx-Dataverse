...
namespace Microsoft.PowerFx.Dataverse
{
    ...
    {        
        ...
        public virtual async Task<DataverseResponse<Entity>> RetrieveAsync(string logicalName, Guid id, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            return DataverseExtensions.DataverseCall(() => OrganizationService.Retrieve(logicalName, id, new ColumnSet(true)), $"Retrieve '{logicalName}':{id}");
        }
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            return DataverseExtensions.DataverseCall((Func<bool>)(() => { this.OrganizationService.Update(entity); return true; }), $"Update '{entity.LogicalName}':{entity.Id}");
        }
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            return DataverseExtensions.DataverseCall(() => OrganizationService.RetrieveMultiple(query), $"Query {query.ToString()} returned nothing");
        }
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();            
            return DataverseExtensions.DataverseCall((Func<bool>)(() => { this.OrganizationService.Delete(entityName, id); return true; }), $"Delete '{entityName}':{id}");
        }
