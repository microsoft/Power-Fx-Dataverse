...
namespace Microsoft.PowerFx.Dataverse.Tests.DelegationTests
{
    public class LookUpDelegationTests
    {
        ...
        //Basic case
        [InlineData("LookUp(t1, Price = 255).Price", null, 1, true, true)]
        [InlineData("LookUp(t1, Price = 255).Price", null, 2, false, false)]
        [InlineData("LookUp(t1, Price = 255).Price", null, 3, true, false)]
        [InlineData("LookUp(t1, Price = 255).Price", null, 4, false, true)]
        [InlineData("LookUp(t1, IsBlank(Price)).Price", null, 5, true, true)]

        // can't delegate IsBlank, because inside is non delegable.
        [InlineData("LookUp(t1, IsBlank(Price * Price)).Price", null, 6, true, true,
            @"Warning 11-33: Delegation warning. The ""IsBlank"" part of this formula might not work correctly on large data sets.",
            @"Warning 7-9: This operation on table 'local' may not work if it has more than 999 rows.")]
        [InlineData("LookUp(t1, IsBlank(LookUp(t1, IsBlank(Price)))).Price", 100.0, 7, true, true,
            "Warning 7-9: This operation on table 'local' may not work if it has more than 999 rows.")]

        [InlineData("LookUp(t1, Integer = 255).Price", null, 8, true, true)]
        [InlineData("LookUp(t1, Integer = 255).Price", null, 9, false, false)]
        [InlineData("LookUp(t1, Integer = 255).Price", null, 10, true, false)]
        [InlineData("LookUp(t1, Integer = 255).Price", null, 11, false, true)]

        [InlineData("LookUp(t1, localid=GUID(\"00000000-0000-0000-0000-000000000001\")).Price", 100.0, 12, true, true)]

        //Basic case with And and Or
        [InlineData("LookUp(t1, localid=GUID(\"00000000-0000-0000-0000-000000000001\") And Price > 0).Price", 100.0, 13, true, true)]
        [InlineData("LookUp(t1, localid=GUID(\"00000000-0000-0000-0000-000000000001\") And Price > 0).Price", 100.0, 14, false, false)]
        [InlineData("LookUp(t1, localid=GUID(\"00000000-0000-0000-0000-000000000001\") And Price > 0).Price", 100.0, 15, true, false)]
        [InlineData("LookUp(t1, localid=GUID(\"00000000-0000-0000-0000-000000000001\") And Price > 0).Price", 100.0, 16, false, true)]

        [InlineData("LookUp(t1, LocalId=GUID(\"00000000-0000-0000-0000-000000000001\") Or Price > 0).Price", 100.0, 17, true, true)]
        [InlineData("LookUp(t1, LocalId=GUID(\"00000000-0000-0000-0000-000000000001\") Or Price > 0).Price", 100.0, 18, false, false)]
        [InlineData("LookUp(t1, LocalId=GUID(\"00000000-0000-0000-0000-000000000001\") Or Price > 0).Price", 100.0, 19, true, false)]
        [InlineData("LookUp(t1, LocalId=GUID(\"00000000-0000-0000-0000-000000000001\") Or Price > 0).Price", 100.0, 20, false, true)]

        // variable
        [InlineData("LookUp(t1, LocalId=_g1).Price", 100.0, 21, true, true)]

        // Date
        [InlineData("LookUp(t1, Date = Date(2023, 6, 1)).Price", 100.0, 22, true, true)]
        ...
        [InlineData("LookUp(t1, Date = Date(2023, 6, 1)).Price", 100.0, 23, false, false)]

        // DateTime with coercion
        [InlineData("LookUp(t1, DateTime = Date(2023, 6, 1)).Price", null, 24, true, true)]
        [InlineData("LookUp(t1, DateTime = DateTime(2023, 6, 1, 12, 0, 0)).Price", 100.0, 25, true, true)]
        [InlineData("LookUp(t1, DateTime = DateTime(2023, 6, 1, 12, 0, 0)).Price", 100.0, 26, false, false)]

        // reversed order still ok
        [InlineData("LookUp(t1, _g1 = LocalId).Price", 100.0, 27, true, true)]

        // explicit ThisRecord is ok. IR will handle.
        [InlineData("LookUp(t1, ThisRecord.LocalId=_g1).Price", 100.0, 28, true, true)]

        // Alias is ok. IR will handle.
        [InlineData("LookUp(t1 As XYZ, XYZ.LocalId=_g1).Price", 100.0, 29, true, true)] // variable

        // lambda uses ThisRecord.Price, can't delegate
        [InlineData("LookUp(t1, LocalId=If(Price>50, _g1, _gMissing)).Price", 100.0, 30, true, true, 
            @"Warning 18-19: Delegation warning. The ""LookUp"" part of this formula might not work correctly on large data sets.",
            @"Warning 19-47: Delegation warning. The ""If"" part of this formula might not work correctly on large data sets.",
            @"Warning 22-27: Can't delegate LookUp: Expression compares multiple fields.")]
        [InlineData("LookUp(t1, LocalId=If(Price>50, _g1, _gMissing)).Price", 100.0, 31, false, false,
            @"Warning 18-19: Delegation warning. The ""LookUp"" part of this formula might not work correctly on large data sets.",
            @"Warning 19-47: Delegation warning. The ""If"" part of this formula might not work correctly on large data sets.",
            @"Warning 22-27: Can't delegate LookUp: Expression compares multiple fields.")]
        [InlineData("LookUp(t1, LocalId=If(Price>50, _g1, _gMissing)).Price", 100.0, 32, true, false,
            @"Warning 18-19: Delegation warning. The ""LookUp"" part of this formula might not work correctly on large data sets.",
            @"Warning 19-47: Delegation warning. The ""If"" part of this formula might not work correctly on large data sets.",
            @"Warning 22-27: Can't delegate LookUp: Expression compares multiple fields.")]
        [InlineData("LookUp(t1, LocalId=If(Price>50, _g1, _gMissing)).Price", 100.0, 33, false, true,
            @"Warning 18-19: Delegation warning. The ""LookUp"" part of this formula might not work correctly on large data sets.",
            @"Warning 19-47: Delegation warning. The ""If"" part of this formula might not work correctly on large data sets.",
            @"Warning 22-27: Can't delegate LookUp: Expression compares multiple fields.")]

        [InlineData("With({r: t1}, LookUp(r, LocalId=If(Price>50, _g1, _gMissing)).Price)", 100.0, 34, true, true,
            "Warning 35-40: Can't delegate LookUp: Expression compares multiple fields.")]
        [InlineData("With({r: t1}, LookUp(r, LocalId=If(Price>50, _g1, _gMissing)).Price)", 100.0, 35, false, false,
            "Warning 35-40: Can't delegate LookUp: Expression compares multiple fields.")]
        [InlineData("With({r: t1}, LookUp(r, LocalId=If(Price>50, _g1, _gMissing)).Price)", 100.0, 36, true, false,
            "Warning 35-40: Can't delegate LookUp: Expression compares multiple fields.")]
        [InlineData("With({r: t1}, LookUp(r, LocalId=If(Price>50, _g1, _gMissing)).Price)", 100.0, 37, false, true,
            "Warning 35-40: Can't delegate LookUp: Expression compares multiple fields.")]

        // On non primary field.
        [InlineData("LookUp(t1, Price > 50).Price", 100.0, 38, true, true)]
        [InlineData("LookUp(t1, Price > 50).Price", 100.0, 39, false, false)]
        [InlineData("LookUp(t1, Price > 50).Price", 100.0, 40, true, false)]
        [InlineData("LookUp(t1, Price > 50).Price", 100.0, 41, false, true)]

        // successful with complex expression
        [InlineData("LookUp(t1, LocalId=If(true, _g1, _gMissing)).Price", 100.0, 42, true, true)]

        // nested delegation, both delegated.
        [InlineData("LookUp(t1, LocalId=LookUp(t1, LocalId=_g1).LocalId).Price", 100.0, 43, true, true)]

        // Can't delegate if Table Arg is delegated.
        [InlineData("LookUp(Filter(t1, Price = 100), localid=_g1).Price", 100.0, 44, true, true,
            "Warning 14-16: This operation on table 'local' may not work if it has more than 999 rows.")]
        [InlineData("LookUp(Filter(t1, Price = 100), localid=_g1).Price", 100.0, 45, false, false,
            "Warning 14-16: This operation on table 'local' may not work if it has more than 999 rows.")]
        [InlineData("LookUp(Filter(t1, Price = 100), localid=_g1).Price", 100.0, 46, true, false,
            "Warning 14-16: This operation on table 'local' may not work if it has more than 999 rows.")]
        [InlineData("LookUp(Filter(t1, Price = 100), localid=_g1).Price", 100.0, 47, false, true,
            "Warning 14-16: This operation on table 'local' may not work if it has more than 999 rows.")]

        [InlineData("With({r: t1}, LookUp(Filter(r, Price = 100), localid=_g1).Price)", 100.0, 48, true, true,
            "Warning 9-11: This operation on table 'local' may not work if it has more than 999 rows.")]
        [InlineData("With({r: t1}, LookUp(Filter(r, Price = 100), localid=_g1).Price)", 100.0, 49, false, false,
            "Warning 9-11: This operation on table 'local' may not work if it has more than 999 rows.")]
        [InlineData("With({r: t1}, LookUp(Filter(r, Price = 100), localid=_g1).Price)", 100.0, 50, true, false,
            "Warning 9-11: This operation on table 'local' may not work if it has more than 999 rows.")]
        [InlineData("With({r: t1}, LookUp(Filter(r, Price = 100), localid=_g1).Price)", 100.0, 51, false, true,
            "Warning 9-11: This operation on table 'local' may not work if it has more than 999 rows.")]

        // Can't delegate if Table Arg is delegated.
        [InlineData("LookUp(FirstN(t1, 1), localid=_g1).Price", 100.0, 52, true, true,
            "Warning 14-16: This operation on table 'local' may not work if it has more than 999 rows.")]
        [InlineData("With({r: t1}, LookUp(FirstN(r, 1), localid=_g1).Price)", 100.0, 53, true, true,
            "Warning 9-11: This operation on table 'local' may not work if it has more than 999 rows.")]

        // Can Delegate on non primary-key field.
        [InlineData("LookUp(t1, LocalId=LookUp(t1, ThisRecord.Price>50).LocalId).Price", 100.0, 54, true, true)]
        [InlineData("LookUp(t1, LocalId=LookUp(t1, ThisRecord.Price>50).LocalId).Price", 100.0, 55, false, false)]
        [InlineData("LookUp(t1, LocalId=LookUp(t1, ThisRecord.Price>50).LocalId).Price", 100.0, 56, true, false)]
        [InlineData("LookUp(t1, LocalId=LookUp(t1, ThisRecord.Price>50).LocalId).Price", 100.0, 57, false, true)]

        [InlineData("LookUp(t1, LocalId=First([_g1,_gMissing]).Value).Price", 100.0, 58, true, true)]

        // unsupported function, can't yet delegate
        [InlineData("Last(t1).Price", -10.0, 59, true, true,
            "Warning 5-7: This operation on table 'local' may not work if it has more than 999 rows.")]

        // unsupported function, can't yet delegate
        [InlineData("CountRows(t1)", 3.0, 60, true, true,
        ...
        [InlineData("IsBlank(t1)", false, /* nothing to delegate */ 61, true, true)]
        [InlineData("IsBlank(Filter(t1, 1=1))", false, /* nothing to delegate */ 62, true, true,
            "Warning 15-17: This operation on table 'local' may not work if it has more than 999 rows.")]
        [InlineData("IsBlank(Filter(t1, 1=1))", false, /* nothing to delegate */ 63, false, false,
            "Warning 15-17: This operation on table 'local' may not work if it has more than 999 rows.")]
        [InlineData("IsBlank(Filter(t1, 1=1))", false, /* nothing to delegate */ 64, true, false,
            "Warning 15-17: This operation on table 'local' may not work if it has more than 999 rows.")]
        [InlineData("IsBlank(Filter(t1, 1=1))", false, /* nothing to delegate */ 65, false, true,
            "Warning 15-17: This operation on table 'local' may not work if it has more than 999 rows.")]

        [InlineData("Collect(t1, { Price : 200}).Price", 200.0, /* Collect shouldn't give warnings. */ 66, true, true)]
        [InlineData("Collect(t1, { Price : 200}).Price", 200.0, /* Collect shouldn't give warnings. */ 67, false, false)]
        [InlineData("Collect(t1, { Price : 200}).Price", 200.0, /* Collect shouldn't give warnings. */ 68, true, false)]
        [InlineData("Collect(t1, { Price : 200}).Price", 200.0, /* Collect shouldn't give warnings. */ 69, false, true)]

        // $$$ Confirm is NotFound Error or Blank?
        [InlineData("IsError(LookUp(t1, LocalId=If(false, _g1, _gMissing)))", true, /* delegated, but not found is Error */ 70, true, true)]

        // $$$ Does using fakeT1, same as t1, cause warnings since it's not delegated?
        [InlineData("LookUp(fakeT1, LocalId=_g1).Price", 100.0, 71, true, true)] // variable
        [InlineData("LookUp(t1, LocalId=LocalId).Price", 100.0, 72, true, true,
        ...
        [InlineData("With({r: t1}, LookUp(r, LocalId=LocalId).Price)", 100.0, 73, true, true,
        ...
        // Error Handling
        [InlineData("LookUp(t1, Price = If(1/0, 255)).Price", typeof(ErrorValue), 74, true, true)]
        [InlineData("LookUp(t1, Price = If(1/0, 255)).Price", typeof(ErrorValue), 75, false, false)]
        [InlineData("LookUp(t1, Price = If(1/0, 255)).Price", typeof(ErrorValue), 76, true, false)]
        [InlineData("LookUp(t1, Price = If(1/0, 255)).Price", typeof(ErrorValue), 77, false, true)]

        // Blank Handling
        [InlineData("LookUp(t1, Price = Blank()).Price", null, 78, true, true)]
        [InlineData("LookUp(t1, Price = Blank()).Price", null, 79, false, false)]
        [InlineData("LookUp(t1, Price = Blank()).Price", null, 80, true, false)]
        [InlineData("LookUp(t1, Price = Blank()).Price", null, 81, false, true)]

        [InlineData("LookUp(t1, Price <> Blank()).Price", 100.0, 82, true, true)]
        [InlineData("LookUp(t1, Price <> Blank()).Price", 100.0, 83, false, false)]
        [InlineData("LookUp(t1, Price <> Blank()).Price", 100.0, 84, true, false)]
        [InlineData("LookUp(t1, Price <> Blank()).Price", 100.0, 85, false, true)]

        [InlineData("LookUp(t1, Price < Blank()).Price", -10.0, 86, true, true)]
        [InlineData("LookUp(t1, Price < Blank()).Price", -10.0, 87, false, false)]
        [InlineData("LookUp(t1, Price < Blank()).Price", -10.0, 88, true, false)]
        [InlineData("LookUp(t1, Price < Blank()).Price", -10.0, 89, false, true)]

        [InlineData("LookUp(t1, Currency > 0).Price", 100.0, 90, true, true)]
        [InlineData("LookUp(t1, Currency > 0).Price", 100.0, 91, false, false)]
        [InlineData("LookUp(t1, Currency > 0).Price", 100.0, 92, true, false)]
        [InlineData("LookUp(t1, Currency > 0).Price", 100.0, 93, false, true)]

        [InlineData("With({r: t1}, LookUp(r, Currency > 0).Price)", 100.0, 94, true, true)]
        [InlineData("With({r: t1}, LookUp(r, Currency > 0).Price)", 100.0, 95, false, false)]
        [InlineData("With({r: t1}, LookUp(r, Currency > 0).Price)", 100.0, 96, true, false)]
        [InlineData("With({r: t1}, LookUp(r, Currency > 0).Price)", 100.0, 97, false, true)]

        [InlineData("With({r : t1}, LookUp(r, LocalId=_g1).Price)", 100.0, 98, true, true)]

        [InlineData("With({r : Filter(t1, Price < 120)}, LookUp(r, Price > 90).Price)", 100.0, 99, false, false,
            "Warning 17-19: This operation on table 'local' may not work if it has more than 999 rows.")]

        [InlineData("With({r: t1} ,LookUp(r, localid=GUID(\"00000000-0000-0000-0000-000000000001\")).Price)", 100.0, 100, true, true)]

        [InlineData("LookUp(t1, virtual.'Virtual Data' = 10).Price", 100.0, 101, true, true)]
        [InlineData("LookUp(t1, virtual.'Virtual Data' = 10).Price", 100.0, 102, false, false)]
        [InlineData("LookUp(t1, virtual.'Virtual Data' = 10).Price", 100.0, 103, true, false)]
        [InlineData("LookUp(t1, virtual.'Virtual Data' = 10).Price", 100.0, 104, false, true)]

        [InlineData("LookUp(t1, virtual.'Virtual Data' <> 10).Price", 10.0, 105, true, true)]
        [InlineData("LookUp(t1, virtual.'Virtual Data' <> 10).Price", 10.0, 106, false, false)]
        [InlineData("LookUp(t1, virtual.'Virtual Data' <> 10).Price", 10.0, 107, true, false)]
        [InlineData("LookUp(t1, virtual.'Virtual Data' <> 10).Price", 10.0, 108, false, true)]

        [InlineData("LookUp(t1, virtual.'Virtual Data' <> 10 And Price <> 10).Price", -10.0, 109, true, true)]
        [InlineData("LookUp(t1, virtual.'Virtual Data' <> 10 And Price <> 10).Price", -10.0, 110, false, false)]
        ...
        [InlineData("LookUp(t1, AsType(PolymorphicLookup, t2).Data = 200).Price", 100.0, 114, true, true,
            ...
            @"Warning 46-47: Delegation warning. The ""LookUp"" part of this formula might not work correctly on large data sets.")]
        [InlineData("LookUp(t1, AsType(PolymorphicLookup, t2).Data = 200).Price", 100.0, 115, false, false,
            @"Warning 40-45: Delegation warning. The highlighted part of this formula might not work correctly with column ""AsType.data"" on large data sets.",
            @"Warning 46-47: Delegation warning. The ""LookUp"" part of this formula might not work correctly on large data sets.")]
        [InlineData("LookUp(t1, AsType(PolymorphicLookup, t2).Data = 200).Price", 100.0, 116, true, false,
            @"Warning 40-45: Delegation warning. The highlighted part of this formula might not work correctly with column ""AsType.data"" on large data sets.",
            @"Warning 46-47: Delegation warning. The ""LookUp"" part of this formula might not work correctly on large data sets.")]
        [InlineData("LookUp(t1, AsType(PolymorphicLookup, t2).Data = 200).Price", 100.0, 117, false, true,
            @"Warning 40-45: Delegation warning. The highlighted part of this formula might not work correctly with column ""AsType.data"" on large data sets.",
            @"Warning 46-47: Delegation warning. The ""LookUp"" part of this formula might not work correctly on large data sets.")]

        [InlineData("LookUp(t1, PolymorphicLookup = First(t2)).Price", 100.0, 118, true, true)]
        [InlineData("LookUp(t1, PolymorphicLookup = First(t2)).Price", 100.0, 119, false, false)]
        [InlineData("LookUp(t1, PolymorphicLookup = First(t2)).Price", 100.0, 120, true, false)]
        [InlineData("LookUp(t1, PolymorphicLookup = First(t2)).Price", 100.0, 121, false, true)]

        [InlineData("LookUp(t1, PolymorphicLookup <> First(t2)).Price", 10.0, 122, true, true)]
        [InlineData("LookUp(t1, PolymorphicLookup <> First(t2)).Price", 10.0, 123, false, false)]
        [InlineData("LookUp(t1, PolymorphicLookup <> First(t2)).Price", 10.0, 124, true, false)]
        [InlineData("LookUp(t1, PolymorphicLookup <> First(t2)).Price", 10.0, 125, false, true)]

        [InlineData("LookUp(Distinct(t1, new_quantity), Value < 20).Value", 10.0, 126, true, true)]
        [InlineData("LookUp(Distinct(t1, new_quantity), Value < 20).Value", 10.0, 127, false, false)]
        [InlineData("LookUp(Distinct(t1, new_quantity), Value < 20).Value", 10.0, 128, true, false)]
        [InlineData("LookUp(Distinct(t1, new_quantity), Value < 20).Value", 10.0, 129, false, true)]

        // Blank handling for retrieveGUID.
        [InlineData("LookUp(t1, localid= If(1<0, GUID(\"00000000-0000-0000-0000-000000000001\"))).Price", null, 130, true, true)]

        // Error Handling.
        [InlineData("LookUp(t1, localid= If(1/0, GUID(\"00000000-0000-0000-0000-000000000001\"))).Price", typeof(ErrorValue), 131, true, true)]
        ...
        [InlineData("LookUp(et, 'Partition Id' = LookUp(t1, Name = \"p1\").Name And etid = GUID(\"00000000-0000-0000-0000-000000000007\")).Field1", 200.0, 164, true, true,
            "#ONLY_WHEN_WITH_TRANSFORM#",
            @"Warning 41-42: Delegation warning. The ""LookUp"" part of this formula might not work correctly on large data sets.",
            @"Warning 43-65: Delegation warning. The ""LookUp"" part of this formula might not work correctly on large data sets.",
            @"Warning 65-70: Delegation warning. The ""LookUp"" part of this formula might not work correctly on large data sets.",
            @"Warning 71-74: Delegation warning. The ""LookUp"" part of this formula might not work correctly on large data sets.")]
        ...
        public async Task LookUpDelegationAsync(string expr, object expected, int id, bool cdsNumberIsFloat, bool parserNumberIsFloatOption, params string[] expectedWarnings)
        {
            ...
            var ewCount = expectedWarnings?.Count() ?? 0;
            var onlyWhenWithTransform = ewCount > 0 && expectedWarnings[0] == "#ONLY_WHEN_WITH_TRANSFORM#";
            if (onlyWhenWithTransform)
            {
                ewCount--;
                expectedWarnings = expectedWarnings.Skip(1).ToArray();
            }
            IList<string> inputs = DelegationTestUtility.TransformForWithFunction(expr, onlyWhenWithTransform ? 0 : ewCount);
            ...
            for (var i = 0; i < inputs.Count; i++)
            {
                ...
                Assert.True((onlyWhenWithTransform && i == 0 ? 0 : ewCount) == errorList.Length, $"expected = {ewCount}, current = {errorList.Length}, i = {i}, expr = {expr}");
        ...
        [InlineData("LookUp(t1, LocalId=If(Price>50, _g1, _gMissing)).Price",
            @"Warning 18-19: Aviso de delegação. A parte ""LookUp"" desta fórmula pode não funcionar corretamente em grandes conjuntos de dados.",
            @"Warning 19-47: Aviso de delegação. A parte ""If"" desta fórmula pode não funcionar corretamente em grandes conjuntos de dados.",
