...
namespace Microsoft.PowerFx.Dataverse.Tests.DelegationTests
{
    public partial class DelegationTests
    {
        ...
        [InlineData(73, "Filter(t1, Price < 200 And IsBlank(Old_Price))", 2, false, false)]
        [InlineData(74, "Filter(t1, Price < 200 And IsBlank(Old_Price))", 2, true, true)]
        [InlineData(75, "Filter(t1, Price < 200 And IsBlank(Old_Price))", 2, true, false)]
        [InlineData(76, "Filter(t1, Price < 200 And IsBlank(Old_Price))", 2, false, true)]
        ...
        [InlineData(194, "Filter(t1, State = If(1<0, 'State (Locals)'.Active))", 2, true, true)]
        [InlineData(195, "Filter(t1, State = If(1<0, 'State (Locals)'.Active))", 2, false, false)]
        [InlineData(196, "Filter(t1, State = If(1<0, 'State (Locals)'.Active))", 2, true, false)]
        [InlineData(197, "Filter(t1, State = If(1<0, 'State (Locals)'.Active))", 2, false, true)]
